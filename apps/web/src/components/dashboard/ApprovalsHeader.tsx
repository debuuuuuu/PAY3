...
export function ApprovalsHeader({
  ...
  const pendingEl = useRef<HTMLSpanElement>(null);
  ...
  useGSAP(
    () => {
      ...
      if (reduced) {
        if (pendingEl.current && pendingCount != null) {
          pendingEl.current.textContent = String(pendingCount);
        }
        return;
      }
    ...
    { scope: root, dependencies: [pendingCount, historyCount] }
  );
  ...
  return (
    ...
    >
      <div
        data-hero-beam
        className="overview-hero-beam pointer-events-none absolute inset-x-6 bottom-0 h-px origin-left bg-gradient-to-r from-transparent via-amber-400/35 to-transparent md:inset-x-10"
        aria-hidden
      />
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div data-overview-block>
          ...
          >
            Transfers above your session threshold wait here. They expire after
            two minutes if you do nothing.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5" data-overview-block>
          ...
          >
            ...
            <p className="mt-1 font-[family-name:var(--font-space-grotesk)] text-[1.35rem] font-semibold tabular-nums text-white">
              <span ref={pendingEl}>{pendingCount ?? "—"}</span>
            </p>
          </div>
          <div
            data-hero-stat
